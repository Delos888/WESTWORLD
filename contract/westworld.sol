...
contract WESTWORLD is Ownable, SeroInterface {
    ...
    uint256 private constant lockPeriod = 30 * 24 * 60 * 60;
    uint256 private constant closurePeriod = 6 * 60 * 60;
    string private constant SERO_CURRENCY = "SERO";
    uint256 private constant maxAmount = 1e23;
    uint256 private constant minAmount = 1e20;
    uint256 private constant tenThousand = 1e22;
    uint256 private constant maxLevels = 10;
    ...
    event MarketshipTransferred(address indexed previousMarketAddr, address indexed newMarketAddr);

    constructor(address _marketAddr, address _codeServiceAddr) public {
        marketAddr = _marketAddr;
        codeService = CodeService(_codeServiceAddr);
        investors.push(Investor({id : 0, parentId : 0, canWithdrawValue : 0,
    ...
    function registerNode(address addr) public onlyOwner {
        ...
        indexs[addr] = index;
        investors.push(Investor({id : index, parentId : 0, canWithdrawValue : 0,
            returnIndex : 0, values : new uint256[](0), timestamps : new uint256[](0), subordinateInfo : SubordinateInfo({counts : new uint256[](maxLevels), amounts : new uint256[](maxLevels), rewards : new uint256[](maxLevels), childsCode : ""})
            }));
    }

    function transferMarketship(address newMarketAddr) public onlyOwner {
        require(newMarketAddr != address(0));
        emit MarketshipTransferred(marketAddr, newMarketAddr);
    ...
    function details(string memory code) public view returns (string selfCode, string parentCode, uint256 canWithdraw, uint256[] values, uint256[] timestamps, uint256 returnIndex) {
        ...
        if (!Utils._stringEq(code, "")) {
            ...
            while (id != indexs[msg.sender]) {
                if (id == 0) {
                    return ("", "", 0, new uint256[](0), new uint256[](0), 0);
                }
    ...
    function withdraw() public {
        ...
        if (closureTime == 0) {
            if (!flag) {
                ...
                while (currentId != 0 && currentLayer < maxLevels) {
    ...
    function canWithdrawCash(Investor storage self) internal view returns (bool flag, uint256 amount, uint256 returnIndex, uint256 capital) {
        flag = true;
        uint256 balance = sero_balanceOf(SERO_CURRENCY);
        if (closureTime != 0 && now > closureTime) {
            ...
            if (self.values.length > 0) {
                for (returnIndex = self.returnIndex; returnIndex < self.values.length; returnIndex++) {
                    ...
                    amount = amount.add(value).add(value.mul(30).div(1000));
    ...
    function calceShareReward(Investor storage ancestor, uint256 height, uint256 childAmount, uint256 childValue) internal view returns (uint256) {
        ...
        if (ancestorAmount <= childAmount) {
            ...
            if (height == 0) {
                return validAmount.mul(3).div(200);
            } else if(0 < height && height <= 4) {
                return validAmount.mul(3).div(2000);
            }else{
                return validAmount.mul(3).div(10000);
            }
        }
    }
    ...
    function register(string memory code) internal {
        ...
        investors.push(Investor({id : index, parentId : parentIndex, canWithdrawValue : 0,
            returnIndex : 0, values : new uint256[](0), timestamps : new uint256[](0), subordinateInfo : SubordinateInfo({counts : new uint256[](maxLevels), amounts : new uint256[](maxLevels), rewards : new uint256[](maxLevels), childsCode : ""})
            }));
    }
    ...
    function invest(string memory code) public payable {
        ...
        uint256 selfAmount;
        if (self.values.length > 0) {
            for (uint256 i = self.values.length; i > self.returnIndex; i--) {
    ...
    function investValue(bool isNew, Investor storage self, uint256 value, uint256 selfAmount) internal {
        ...
        require(sero_send_token(owner, SERO_CURRENCY, value.div(100).mul(2)));
        require(sero_send_token(marketAddr, SERO_CURRENCY, value.div(100)));

        if (self.parentId > 0) {
            Investor storage parent = investors[self.parentId];

            uint256 reward = calceShareReward(parent, 0, selfAmount, value);
            if (reward > 0) {
                parent.canWithdrawValue = parent.canWithdrawValue.add(reward);
            ...
            while (currentId != 0 && height < maxLevels) {
                Investor storage current = investors[currentId];
                uint256 level = current.subordinateInfo.amounts[0].div(tenThousand);
                if (level > height) {
                    reward = calceShareReward(current, height, selfAmount, value);
                    if (reward > 0) {
