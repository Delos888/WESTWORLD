...
import westworld from "./westworld";
import BigNumber from 'bignumber.js'
import {formatDate, decimals} from './utils'
import language from './language'
import WestWorld1_02 from '../img/WestWorld1_02.jpg'
...
class Accounts extends Component {
    ...
    componentDidMount() {
        ...
        westworld.OnInit
            .then(()=>{
                westworld.accountList(function (accounts) {
                    self.initAccount(accounts[0]);
                    setInterval(function () {
                        westworld.accountDetails(self.state.account.pk, function (account) {
    ...
    initAccount(account) {
        let self = this;
        if (account) {
            westworld.details("", account.mainPKr, function (details) {
    ...
    initInfo() {
        let self = this;
        westworld.info(this.state.account.mainPKr, function (info) {
    ...
    invest() {
        ...
        if (value < 100 || value > 100000) {
            alert("value must in [100,100000]")
            return
        }
        let code = "";
        ...
        westworld.invest(this.state.account.pk, this.state.account.mainPKr, value, code, function (ret) {
        });
    }

    withdraw() {
        if (this.state.account.details.canWithdraw !== "0") {
            westworld.withdraw(this.state.account.pk, this.state.account.mainPKr, function (ret) {
            });
        }
    }

    takePartIn() {
        let defCode = "2MM5NXX1L5";
        ...
        let inputs = <div>
            ...
            <InputItem type='money' clear moneyKeyboardAlign='left' ref={el => {
                this.valueInput = el
            }} placeholder="100 ~ 100000"><span>{language.e().account.modal.value}:</span></InputItem>
        </div>
    ...
    render() {
        ...
        let recordItems = this.state.account.details.records.map(
            (record, index) => {
                ...
                let profit = record.value.multipliedBy(30).div(1000);
                let days = 30;
                ...
                return <List.Item key={index}>
                    ...
                    <div style={{float: "left", width: '22%', textAlign: 'center'}}><span
                        className="column-title">{30 - days}</span></div>
                    <div style={{float: "left", width: '22%', textAlign: 'center'}}><span
        ...
        return (
            <div style={{maxWidth: '600px'}}>
                ...
                <div className="header">
                    <img src={WestWorld1_02} width="100%"/><br/>
                    {
